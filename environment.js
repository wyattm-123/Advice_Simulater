class Character {
    ...
    update() {
        ...
        this.animation.bobble = Math.sin(Date.now() / 800) * 0.1;
        
        // Decay squash and stretch
        if (this.animation.squash > 0) {
            this.animation.squash *= 0.9;
            if (this.animation.squash < 0.01) this.animation.squash = 0;
        }
        if (this.animation.stretch > 0) {
            this.animation.stretch *= 0.9;
            if (this.animation.stretch < 0.01) this.animation.stretch = 0;
        }
        if (this.animation.rotation !== 0) {
            this.animation.rotation *= 0.9;
            if (Math.abs(this.animation.rotation) < 0.01) this.animation.rotation = 0;
        }
        
        // Update fun effects
        this.updateFunEffects();
    ...
    getAttention() {
        if (this.interaction.clickable) {
            ...
            // Add some squash and stretch animation
            this.animation.squash = 0.2;
            this.animation.stretch = 0.2;
            this.animation.rotation = (Math.random() - 0.5) * 0.3;
            
            // Generate fun effects
            this.generateFunEffects();
            ...
            // Make character say something funny or random
            ...
            // Pick a funny reaction based on character personality
            let reactionPool = funnyReactions;
            if (this.sprite.characterType === "grumpy") {
                reactionPool = ["Hmph!", "Go away!", "What now?", "*sighs*", "Not you again...", "Do you mind?"];
            } else if (this.sprite.characterType === "fitness") {
                reactionPool = ["Feel the burn!", "Let's go!", "One more rep!", "You got this!", "Woohoo!", "Energy!"];
            } else if (this.sprite.characterType === "lazy") {
                reactionPool = ["*yawns*", "Five more minutes...", "So tired...", "Nap time?", "Why..."];
            }
            
            this.speechBubble.text = reactionPool[Math.floor(Math.random() * reactionPool.length)];
            
            // Play a random emote above the character
            const randomEmoji = this.getRandomEmoji();
            this.showFloatingEmoji(randomEmoji);
    ...
    showFloatingEmoji(emoji) {
        this.animation.funEffects.emoji = emoji;
        this.animation.funEffects.startTime = Date.now();
        this.animation.funEffects.isActive = true;
    }
    
    generateFunEffects() {
        // Generate confetti particles
        this.animation.funEffects.confetti = [];
        this.animation.funEffects.stars = [];
        
        // Create colorful confetti
        for (let i = 0; i < 30; i++) {
            this.animation.funEffects.confetti.push({
                x: this.width / 2,
                y: this.height / 2,
        ...
        // Create stars
        for (let i = 0; i < 15; i++) {
            this.animation.funEffects.stars.push({
                x: this.width / 2,
                y: this.height / 2,
                size: 5 + Math.random() * 10,
                color: `hsl(${Math.floor(Math.random() * 60) + 40}, 100%, 70%)`,
                velocity: {
                    x: (Math.random() - 0.5) * 4,
                    y: -2 - Math.random() * 4
                },
    ...
    updateFunEffects() {
        if (!this.animation.funEffects.isActive) return;
        
        const elapsed = Date.now() - this.animation.funEffects.startTime;
        
        // Check if effects should end
        if (elapsed > this.animation.funEffects.duration) {
            this.animation.funEffects.isActive = false;
            return;
        }
        
        // Update confetti
        this.animation.funEffects.confetti.forEach(particle => {
        ...
        // Update stars
        this.animation.funEffects.stars.forEach(star => {
            star.x += star.velocity.x;
            star.y += star.velocity.y;
            star.velocity.y += 0.05;
            star.opacity = Math.max(0, 1 - elapsed / this.animation.funEffects.duration);
    ...
    drawFunEffects(ctx) {
        ...
        // Draw floating emoji
        if (this.animation.funEffects.emoji) {
            const elapsed = Date.now() - this.animation.funEffects.startTime;
            const progress = elapsed / this.animation.funEffects.duration;
            ...
            const emojiFactor = 1 + Math.sin(progress * Math.PI * 8) * 0.1;
            const emojiY = -30 - progress * 50;
            
            ctx.scale(emojiFactor, emojiFactor);
            ctx.fillText(this.animation.funEffects.emoji, this.width / 2 / emojiFactor, emojiY / emojiFactor);
            ctx.restore();
        }
    }
    ...
    containsPoint(x, y) {
        ...
        // Convert coordinates to character's local space
        const localX = x - this.x;
        const localY = y - this.y;
        
        // Check if point is within the expanded hitbox
        return (
            localX >= -hitboxPadding && 
            localX <= this.width + hitboxPadding && 
            localY >= -hitboxPadding && 
            localY <= this.height + hitboxPadding
        );
    }
    ...
    draw(ctx) {
        ...
        // Apply squash and stretch and rotation
        if (this.animation.squash > 0 || this.animation.stretch > 0 || this.animation.rotation !== 0) {
        ...
        // Apply bobble effect
        if (this.animation.bobble !== 0) {
            ctx.translate(this.width/2, 0);
            ctx.rotate(this.animation.bobble);
            ctx.translate(-this.width/2, 0);
        }
        ...
        // Draw fun effects on top
        this.drawFunEffects(ctx);
        
        ctx.restore();
        
        // Draw speech bubble with subtle shake if excited
        if (this.speechBubble.text && this.speechBubble.visible) {
...
class Environment {
    ...
    update() {
        ...
        // Update mini characters
        this.updateMiniCharacters();
    ...
    draw() {
        ...
        // Draw mini characters after environment but before main characters
        this.drawMiniCharacters(this.ctx);
    ...
    handleMouseDown(event) {
        ...
        if (!foundCharacter) {
            this.createMiniCharacter(mouseX, mouseY);
            this.createRippleEffect(mouseX, mouseY, true);
        }
    }
    ...
    handleMouseMove(event) {
        ...
        this.canvas.style.cursor = this.interaction.isDragging ? 'grabbing' : (isOverCharacter ? 'pointer' : 'default');
        document.body.classList.toggle('character-dragging', this.interaction.isDragging);
    ...
    createRippleEffect(x, y, isColorful = false) {
        const ripple = document.createElement('div');
        ripple.className = isColorful ? 'click-ripple colorful' : 'click-ripple';
    ...
    // Create a fun mini character at the clicked position
    createMiniCharacter(x, y) {
        if (this.interaction.isPlaying) return;
        
        const characterTypes = ["fitness", "lazy", "office", "artistic", "musical", 
                               "comedy", "grumpy", "mediator", "skeptical", "peacemaker"];
        
        const type = characterTypes[Math.floor(Math.random() * characterTypes.length)];
        
        const colors = ['#51cf66', '#748ffc', '#9775fa', '#ffa94d', '#ff8787',
                         '#da77f2', '#66d9e8', '#868e96', '#4dabf7', '#f783ac'];
        ...
        this.interaction.miniCharacters.push(miniChar);
        console.log(`Created mini character at ${x}, ${y}`);
    }
    
    // Update mini characters
    updateMiniCharacters() {
        const now = Date.now();
        for (let i = this.interaction.miniCharacters.length - 1; i >= 0; i--) {
            const mini = this.interaction.miniCharacters[i];
            
            // Apply physics
            mini.x += mini.velocity.x;
            mini.y += mini.velocity.y;
            mini.velocity.y += mini.gravity;
            mini.rotation += mini.rotationSpeed;
            
            // Bounce off ground
            if (mini.y + mini.size > this.canvas.height - this.background.ground.height && mini.bounceCount < mini.maxBounces) {
                mini.velocity.y = -mini.velocity.y * 0.6;
                mini.bounceCount++;
                // Create a small splash effect
                this.createRippleEffect(mini.x, this.canvas.height - this.background.ground.height);
            }
            
            // Bounce off walls
            if (mini.x < 0 || mini.x > this.canvas.width) {
                mini.velocity.x = -mini.velocity.x * 0.8;
            }
            
            // Remove if too old
            if (now - mini.birthTime > mini.lifespan) {
                this.interaction.miniCharacters.splice(i, 1);
            }
        }
    }
    
    // Draw mini characters
    drawMiniCharacters(ctx) {
        ctx.save();
        
        this.interaction.miniCharacters.forEach(mini => {
            // Calculate age factor for fading out
            const age = (Date.now() - mini.birthTime) / mini.lifespan;
            const opacity = Math.max(0, 1 - age * 1.5);
            
            ctx.save();
            ctx.globalAlpha = opacity;
            ctx.translate(mini.x, mini.y);
            ctx.rotate(mini.rotation);
            
            // Draw simple character
            const size = mini.size;
